...
{
    class PutConnectionToggl : IPutConnectionToggl
    {
        public void Put(string apiToken, string url, string json)
        {
            string userpass = apiToken + ":api_token";
            string userpassB64 = Convert.ToBase64String(Encoding.Default.GetBytes(userpass.Trim()));
            json = "{\"project\":{\"name\":\"An awesome project von Test2\",\"wid\":2875373,\"is_private\":true,\"template_id\":131984410}}";
            ...
            Task<HttpResponseMessage> task = httpClient.PutAsync(url, stringContent);
            task.Wait();
